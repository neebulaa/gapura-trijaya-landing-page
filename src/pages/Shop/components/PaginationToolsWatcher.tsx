...
export default function PaginationToolsWatcher({
  ...
  return (
    <section className="pagination-tools-watcher">
      <select id="pagination-tools-perpage" onChange={setTotalItemsPerPage} value={itemsPerPage}>
        <option value={DEFAULT_ITEMS_PER_PAGE}>Show {DEFAULT_ITEMS_PER_PAGE}</option>
      ...
      <section className="btn-split">
        ...
        {new Array(MAX_PAGE_ON_PAGINATION > maxPage ? maxPage : MAX_PAGE_ON_PAGINATION)
          .fill(0)
          .map((_, i) => {
            let page = i + 1;
            const pageMinThreshold =
              MAX_PAGE_ON_PAGINATION - Math.floor(MAX_PAGE_ON_PAGINATION / 2);
            const pageMaxThreshold = maxPage - Math.floor(MAX_PAGE_ON_PAGINATION / 2);
