...
export default function DropdownFilter({
  ...
  function setItem(item: string) {
    ...
    if (type == 'checkbox') {
      if (itemValue.includes(item)) {
        storeItemValue = (itemValue as string[]).filter((pI) => pI != item);
  ...
  return (
    <section className="dropdown-filter">
      <header className="dropdown-filter-header" onClick={toggleOpenDropdown}>
        <h4>{title}</h4>
        <IconChevronDown width={'20'} height={'20'} color={'#737373'} />
      </header>
      <section className={`dropdown-filter-items ${openDropdown ? 'open' : ''}`}>
        {items.slice(0, openAll ? undefined : max).map((item, index) => {
          if (type == 'link')
            return (
              <button
                onClick={() => setItem(item)}
                key={`${componentId} - ${index}`}
                className={`btn-link ${item == currentItem ? 'current' : ''}`}
              >
                {item}
              </button>
            );
